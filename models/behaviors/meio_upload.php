...
class MeioUploadBehavior extends ModelBehavior {
	...
	var $default_options = array(
		...
		'validations' => array(
			'FieldName' => array(
				...
				'check' => true,
				'message' => 'このフィールドは定義されていません。'
			),
			'Dir' => array(
				'rule' => array('uploadCheckDir'),
				'check' => true,
				'message' => 'このファイルが存在しないまたは書き込み保護されている場合、ディレクトリが置かれることになります。'
			),
			'Empty' => array(
			...
			'UploadError' => array(
				...
				'check' => true,
				'message' => 'ファイルをアップロード中に問題がおきました。'
			),
			'MaxSize' => array(
				'rule' => array('uploadCheckMaxSize'),
				'check' => true,
				'message' => 'ファイルの最大サイズを超えています。'
			),
			'InvalidMime' => array(
	 ...
	 */
	var $moveErrorMsg = 'アーカイブコピーの間に問題が起きました。';
	...
	function createthumb($name, $filename, $new_w, $new_h)
	{
	   ...
	   } else if ($old_x >= $old_y)
	   {
		...
		if ($type === IMAGETYPE_GIF || $type === IMAGETYPE_PNG) {
			$transIndex = imagecolortransparent($src_img);
			if ($transIndex >= 0 and $transIndex < imagecolorstotal($src_img)) {
				$transColor = imagecolorsforindex($src_img, $transIndex);
				$transIndex = imagecolorallocate($dst_img, $transColor['red'], $transColor['green'], $transColor['blue']);
	   ...
	   imagecopyresampled($dst_img, $src_img, 0, 0, 0, 0, $thumb_w, $thumb_h, $old_x, $old_y);
		...
		switch ($type) {
			case IMAGETYPE_GIF:
				return imagegif($dst_img, $filename);
				break;
			case IMAGETYPE_JPEG:
				return imagejpeg($dst_img, $filename);
				break;
			case IMAGETYPE_PNG:
				return imagepng($dst_img, $filename);
				break;
		}
